        ...
        <div className='grid gird-row-4'>
            ...
            <div className='text-xl row-span-1 pt-10'>
                {buttonsList.map((buttonData, index) =>
                    <div className={`${selectedButtons.includes(buttonData) ? 'bgClickBlue' : 'bgBlue'} p-4 m-6 rounded-full text-white cursor-pointer`} onClick={() => handleClick(buttonData)}>{buttonData}</div>
                )}
            </div>
